...
function App() {
  ...
  const handleSearchClick = async (searchValue) => {
    ...
    switch (selectedButton) {
      ...
      case 'notícias':
        ...
        break;
      case 'criptoativos':
        const cryptoForexData = await CryptoForexQuotes.getCryptoForexData(searchValue);
        console.log('Crypto data:', cryptoForexData);
        setCryptoForexList((prevResults) => [...prevResults, ...cryptoForexData.data]);
        break;
      default:
  ...
  const renderResults = () => {
    switch(selectedButton) {
      ...
      case 'criptoativos':
        return <CryptoForexSearchResults result={cryptoForexList} />;
      default:
        break;
    }  
  }
    ...
    <div>
      ...
      <Container sx={{ marginTop: '20px' }}>
        ...
        <Box sx={{ display: 'flex', justifyContent: 'center', marginTop: '20px' }}>
          <MainSearchBar selectedButton={selectedButton} handleSearchClick={handleSearchClick} />
        </Box>
      </Container>
